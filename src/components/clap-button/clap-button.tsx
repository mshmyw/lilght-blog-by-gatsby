...
export default () => {
  ...
  const _animationTimeline = new mojs.Timeline();
  const clapEl = useRef(null);
  ...
  useEffect(()=> {
    ...
    clapEl.current.style.transform = "scale(1, 1)"

    _animationTimeline.add([
      countAnimation,
      countTotalAnimation,
      scaleButton,
      circleBurst,
  ...
  const handleClick = () => {
    _animationTimeline.replay();
    setCount(pre => Math.min(pre + 1, 50));
    setCountTotal(pre => pre + 1);
    setIsClicked(true);
  };

  return (<div><button id="clap" ref={ clapEl }  className="clap" onClick={handleClick}>
  <span>
